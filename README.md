...
### Screen (ecall 2000-2003)

MC-side implementation of the standardized screen (reference model's
app/gfx-saver, built in by default, see ``compile.py``).  Generated by
plugin/screen_gen.py.

- Wall is ``screen_width`` x ``screen_height`` px (default 192 x 168;
  PyriscvConfig, see src/python/config.py -- the reference model's
  app/gfx-saver must be rebuilt for a different size), placed at build
  time: ``screen_origin`` is the block of the top-left pixel,
  ``screen_facing`` the direction the screen front points
  (north/south/east/west for walls; up/down for a floor/ceiling, with
  ``screen_top`` choosing the compass direction the picture top faces).
  Defaults reproduce the classic wall at x 0..191, y 64..231, z = 0.
  Set ``screen_enabled`` to false to build the screen out; reload then
  skips screen:init.
- Draw (2000) compares guest memory words (``org_jawbts_riscvmc2_memory``)
  against a per-pixel cache (``org_jawbts_riscvmc2_screen_cache``) and
  setblocks only changed pixels.
- Game tick (2001/2002): ``screen:tick_gt`` (in the ``minecraft:tick``
  tag) adds 1 to ``#game_gt`` (``org_jawbts_riscvmc2_temp``) every game
  tick; gt_wait blocks until the next one, like read.
- Keys (2003) are pressed by setting the scoreboard manually, e.g.
  ``/scoreboard players set #k_0 org_jawbts_riscvmc2_key 1``.  They are
  latched and all reset at the end of each draw.
